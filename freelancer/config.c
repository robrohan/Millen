...
static int vesares[13][2] = {{320, 200}, {360, 200}, {320, 240}, {360, 240},  {320, 400},   {360, 400},  {640, 350},
...
static struct
{
    ...
    {"fullscreen", type_bool, &fullscreen,
     ...
     ";   0 - Windowed\n"
     ";   1 - Fullscreen\n"},
    {"xdim", type_int, &xdimgame, "; Video resolution\n"},
    {"ydim", type_int, &ydimgame, NULL},
    {"bpp", type_int, &bppgame, "; Video colour depth\n"},
#if USE_POLYMOST
    ...
    {"glusetexcache", type_bool, &glusetexcache, "; OpenGL mode options\n"},
#endif
#ifdef RENDERTYPEWIN
    {"maxrefreshfreq", type_int, &maxrefreshfreq, "; Maximum OpenGL mode refresh rate (Windows only, in Hertz)\n"},
#endif
    ...
    {"joystick", type_bool, &tmpjoystick,
     ...
     ";   0 - No\n"
     ";   1 - Yes\n"},
    {"mousesensitivity", type_double, &msens, "; Mouse sensitivity\n"},
    ...
    while (1)
    {
        ...
        for (item = 0; configspec[item].name; item++)
        {
            if (!Bstrcasecmp(token, configspec[item].name))
            {
                ...
                case type_bool: {
                    int value = 0;
                    if (scriptfile_getnumber(cfg, &value))
                        break;
                ...
                case type_hex: {
                    int value = 0;
                    if (scriptfile_gethex(cfg, &value))
                        break;
                    *(int *)configspec[item].store = value;
                    break;
                }
                case type_double: {
                    double value = 0.0;
                    if (scriptfile_getdouble(cfg, &value))
                        break;
