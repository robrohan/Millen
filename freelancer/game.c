...
/***************************************************************************
 ...
 sector[?].lotag = 16  A true double-sliding door.
    ...
    wall[?].lotag = 3   Marked wall to determine starting dir. (sector tag 12)
...
#define spawnsprite(newspriteindex2, x2, y2, z2, cstat2, shade2, pal2, clipdist2, xrepeat2, yrepeat2, xoffset2,        \
                    yoffset2, picnum2, ang2, xvel2, yvel2, zvel2, owner2, sectnum2, statnum2, lotag2, hitag2, extra2)  \
    {                                                                                                                  \
        spritetype *spr2;                                                                                              \
        ...
        spr2->shade = shade2;                                                                                          \
        spr2->pal = pal2;                                                                                              \
        spr2->clipdist = clipdist2;                                                                                    \
        spr2->xrepeat = xrepeat2;                                                                                      \
        spr2->yrepeat = yrepeat2;                                                                                      \
        spr2->xoffset = xoffset2;                                                                                      \
        spr2->yoffset = yoffset2;                                                                                      \
        spr2->picnum = picnum2;                                                                                        \
        spr2->ang = ang2;                                                                                              \
        spr2->xvel = xvel2;                                                                                            \
        spr2->yvel = yvel2;                                                                                            \
...
void gameloop(void)
{
    ...
    {
        while (movefifoplc != movefifoend[0])
            domovethings();
    }
    else
    {
        ...
        if (j == myconnectindex)
            j = connectpoint2[j];
        averagelag[j] =
            ((averagelag[j] * 7 + (((movefifoend[myconnectindex] - movefifoend[j] + otherlag[j] + 2) & 255) << 8)) >>
             3);
...
int app_main(int argc, char const *const argv[])
{
    ...
    OSD_RegisterFunction("vidmode", "vidmode [xdim ydim] [bpp] [fullscreen]: immediately change the video mode",
                         osdcmd_vidmode);
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (argv[i][0] == '-')
        {
            ...
            else if (!Bstrcasecmp(&argv[i][1], "setup"))
                cmdsetup = 1;
    ...
    initsb(option[1], option[2], digihz[option[7] >> 4], ((option[7] & 4) > 0) + 1, ((option[7] & 2) > 0) + 1, 60,
           option[7] & 1);
    ...
    if (option[4] > 0)
    {
        ...
        {
            if (j == 1)
                Bstrcat((char *)getmessage, " (Master)");
            else
                Bstrcat((char *)getmessage, " (Slave)");
        }
        else
...
{ // Door code
    ...
    if (datag == 16) // True sideways double-sliding door
    {
        ...
        for (j = 0; j < 2; j++)
        {
            if ((((wall[wallfind[j]].x + wall[wall[wallfind[j]].point2].x) >> 1) == centx) &&
                (((wall[wallfind[j]].y + wall[wall[wallfind[j]].point2].y) >> 1) == centy))
            { // door was closed
              // find what direction door should open
                ...
                if (dax2 != 0)
                {
                    ...
                    setanimation(&wall[i].x, wall[i].x + dax2, 4L, 0L);
                    setanimation(&wall[wall[wallfind[j]].point2].x, wall[wall[wallfind[j]].point2].x + dax2, 4L, 0L);
                    setanimation(&wall[wall[wall[wallfind[j]].point2].point2].x,
                                 wall[wall[wall[wallfind[j]].point2].point2].x + dax2, 4L, 0L);
                }
                else if (day2 != 0)
                {
                    ...
                    setanimation(&wall[i].y, wall[i].y + day2, 4L, 0L);
                    setanimation(&wall[wall[wallfind[j]].point2].y, wall[wall[wallfind[j]].point2].y + day2, 4L, 0L);
                    setanimation(&wall[wall[wall[wallfind[j]].point2].point2].y,
                                 wall[wall[wall[wallfind[j]].point2].point2].y + day2, 4L, 0L);
                }
            }
            else
            { // door was not closed
...
void operatesprite(short dasprite)
{
    ...
    {
        shootgun(dasprite, sprite[dasprite].x, sprite[dasprite].y, sprite[dasprite].z, sprite[dasprite].ang, 100L,
                 sprite[dasprite].sectnum, 2);
    }
}
...
{ // Andy did this
    ...
    if (nummissiles[snum] > 999)
        nummissiles[snum] = 999;
...
{ // Andy did this
    ...
    if (numgrabbers[snum] > 999)
        numgrabbers[snum] = 999;
...
{ // Andy did this
    ...
    if ((snum == screenpeek) && (screensize <= xdim))
    {
        ...
        if (nstatusflytime > 1000)
            nstatusflytime = 1000;
        else if (nstatusflytime < 0)
            nstatusflytime = 0;
        if (nstatusflytime != ostatusflytime)
        {
            if (nstatusflytime > 999)
...
void prepareboard(char *daboardfilename)
{
    ...
    for (i = 0; i < numsectors; i++)
    {
        ...
        case 15:
            ...
            for (j = 0; j < subwaystopcnt[subwaytrackcnt]; j++)
                if (klabs(subwaystop[subwaytrackcnt][j] - subwayx[subwaytrackcnt]) <
                    klabs(subwaystop[subwaytrackcnt][subwaygoalstop[subwaytrackcnt]] - subwayx[subwaytrackcnt]))
    ...
    {
        if (sprite[i].lotag == 3)
            turnspritelist[turnspritecnt++] = i;
        ...
        if (sprite[i].statnum < MAXSTATUS) // That is, if sprite exists
            ...
            case BROWNMONSTER: // All cases here put the sprite
                ...
                if (sprite[i].statnum != 1)
                    changespritestat(i, 2); // on waiting for you (list 2)
    ...
    for (i = MAXSPRITES - 1; i >= 0; i--)
        copybuf(&sprite[i].x, &osprite[i].x, 3);
...
void checktouchsprite(short snum, short sectnum)
{
    ...
    for (i = headspritesect[sectnum]; i >= 0; i = nexti)
    {
        ...
        if (sprite[i].cstat & 0x8000)
            continue;
        if ((klabs(posx[snum] - sprite[i].x) + klabs(posy[snum] - sprite[i].y) < 512) &&
            (klabs((posz[snum] >> 8) - ((sprite[i].z >> 8) - (tilesizy[sprite[i].picnum] >> 1))) <= 40))
        {
            switch (sprite[i].picnum)
            {
            case COIN:
            ...
            case COINSTACK:
                ...
                break;
            case GIFTBOX:
                wsayfollow("getstuff.wav",
                           4096L + (krand() & 127) + 256 - mulscale4(sprite[i].xrepeat, sprite[i].yrepeat), 208L,
...
void checkgrabbertouchsprite(short snum, short sectnum) // Andy did this
{
    ...
    for (i = headspritesect[sectnum]; i >= 0; i = nexti)
    {
        ...
        if (sprite[i].cstat & 0x8000)
            continue;
        if ((klabs(sprite[snum].x - sprite[i].x) + klabs(sprite[snum].y - sprite[i].y) < 512) &&
            (klabs((sprite[snum].z >> 8) - ((sprite[i].z >> 8) - (tilesizy[sprite[i].picnum] >> 1))) <= 40))
        {
            switch (sprite[i].picnum)
            {
            case COIN:
    ...
    case 0: // Shoot chain gun
        ...
        if (wall[hitwall].picnum == KENPICTURE)
        {
            if (waloff[MAXTILES - 1] != 0)
                wall[hitwall].picnum = MAXTILES - 1;
            wsayfollow("hello.wav", 4096L + (krand() & 127) - 64, 256L, &wall[hitwall].x, &wall[hitwall].y, 0);
        }
        else if (((hitwall < 0) && (hitsprite < 0) && (hitz >= z) &&
                  ((sector[hitsect].floorpicnum == SLIME) || (sector[hitsect].floorpicnum == FLOORMIRROR))) ||
                 ((hitwall >= 0) && (wall[hitwall].picnum == SLIME)))
        { // If you shoot slime, make a splash
            wsayfollow("splash.wav", 4096L + (krand() & 511) - 256, 256L, &hitx, &hity, 0);
            spawnsprite(j, hitx, hity, hitz, 2, 0, 0, 32, 64, 64, 0, 0, SPLASH, daang, 0, 0, 0, snum + 4096, hitsect, 4,
                        63, 0, 0); // 63=time left for splash
        }
        else
        {
            ...
            if ((hitsprite >= 0) && (sprite[hitsprite].statnum < MAXSTATUS))
                ...
                case BROWNMONSTER:
                    ...
                    else
                    {
                        ...
                        spawnsprite(j, hitx, hity, hitz + (32 << 8), 0, -4, 0, 32, 64, 64, 0, 0, EXPLOSION, daang, 0, 0,
                                    0, snum + 4096, hitsect, 5, 31, 0, 0);
                    }
                    break;
                case EVILAL:
                    ...
                    spawnsprite(j, hitx, hity, hitz + (32 << 8), 0, -4, 0, 32, 64, 64, 0, 0, EXPLOSION, daang, 0, 0, 0,
                                snum + 4096, hitsect, 5, 31, 0, 0);
            ...
            spawnsprite(j, hitx, hity, hitz + (8 << 8), 2, -4, 0, 32, 16, 16, 0, 0, EXPLOSION, daang, 0, 0, 0,
                        snum + 4096, hitsect, 3, 63, 0, 0);

            // Sprite starts out with center exactly on wall.
            // This moves it back enough to see it at all angles.
            movesprite((short)j, -(((int)sintable[(512 + daang) & 2047] * TICSPERFRAME) << 4),
                       -(((int)sintable[daang] * TICSPERFRAME) << 4), 0L, 4L << 8, 4L << 8, CLIPMASK1);
        }
        break;
    case 1: // Shoot silver sphere bullet
        spawnsprite(j, x, y, z, 1 + 128, 0, 0, 16, 64, 64, 0, 0, BULLET, daang, sintable[(daang + 512) & 2047] >> 5,
                    sintable[daang & 2047] >> 5, (100 - dahoriz) << 6, snum + 4096, dasectnum, 6, 0, 0, 0);
        wsayfollow("shoot2.wav", 4096L + (krand() & 127) - 64, 128L, &sprite[j].x, &sprite[j].y, 1);
        break;
    case 2: // Shoot bomb
        spawnsprite(j, x, y, z, 128, 0, 0, 12, 16, 16, 0, 0, BOMB, daang, sintable[(daang + 512) & 2047] * 5 >> 8,
                    sintable[daang & 2047] * 5 >> 8, (80 - dahoriz) << 6, snum + 4096, dasectnum, 6, 0, 0, 0);
        wsayfollow("shoot3.wav", 4096L + (krand() & 127) - 64, 192L, &sprite[j].x, &sprite[j].y, 1);
        break;
    case 3: // Shoot missile (Andy did this)
        spawnsprite(j, x, y, z, 1 + 128, 0, 0, 16, 32, 32, 0, 0, MISSILE, daang, sintable[(daang + 512) & 2047] >> 4,
                    sintable[daang & 2047] >> 4, (100 - dahoriz) << 7, snum + 4096, dasectnum, 6, 0, 0, 0);
        wsayfollow("shoot3.wav", 4096L + (krand() & 127) - 64, 192L, &sprite[j].x, &sprite[j].y, 1);
        break;
    case 4: // Shoot grabber (Andy did this)
        spawnsprite(j, x, y, z, 1 + 128, 0, 0, 16, 64, 64, 0, 0, GRABBER, daang, sintable[(daang + 512) & 2047] >> 5,
                    sintable[daang & 2047] >> 5, (100 - dahoriz) << 6, snum + 4096, dasectnum, 6, 0, 0, 0);
    ...
    for (i = 0, tspr = &tsprite[0]; i < spritesortcnt; i++, tspr++)
    {
        ...
        if (k >= 0) // Interpolate moving sprite
        {
            ...
            case 0:
                ...
                break;
            case 1:
                j = (smoothratio >> 1) + (((nummoves - tspr->owner) & 1) << 15);
                break;
            case 3:
                j = (smoothratio >> 2) + (((nummoves - tspr->owner) & 3) << 14);
                break;
            case 7:
                j = (smoothratio >> 3) + (((nummoves - tspr->owner) & 7) << 13);
                break;
            case 15:
                j = (smoothratio >> 4) + (((nummoves - tspr->owner) & 15) << 12);
                break;
            }
            k = tspr->x - ospr->x;
            tspr->x = ospr->x;
            if (k != 0)
                tspr->x += mulscale16(k, j);
            k = tspr->y - ospr->y;
            tspr->y = ospr->y;
            if (k != 0)
                tspr->y += mulscale16(k, j);
            k = tspr->z - ospr->z;
            tspr->z = ospr->z;
            if (k != 0)
                tspr->z += mulscale16(k, j);
        }
...
void tagcode(void)
{
    ...
    for (i = 0; i < swingcnt; i++)
    {
        if (swinganginc[i] != 0)
        {
            ...
            for (k = 1; k <= 3; k++)
                rotatepoint(swingx[i][0], swingy[i][0], swingx[i][k], swingy[i][k], swingang[i],
                            &wall[swingwall[i][k]].x, &wall[swingwall[i][k]].y);
                    ...
                    if ((cursectnum[p] == swingsector[i]) || (testneighborsectors(cursectnum[p], swingsector[i]) == 1))
                    {
                        ...
                        do
                        {
                            ...
                            if (good == 0)
                            {
                                ...
                                if (swingang[i] == swingangclosed[i])
                                {
                                    wsayfollow("closdoor.wav", 4096L + (krand() & 511) - 256, 256L, &swingx[i][0],
                                               &swingy[i][0], 0);
                                    swinganginc[i] = 0;
                                    break;
                                }
    ...
    for (i = 0; i < revolvecnt; i++)
    {
        ...
        for (k = startwall; k < endwall; k++)
        {
            rotatepoint(revolvepivotx[i], revolvepivoty[i], revolvex[i][k - startwall], revolvey[i][k - startwall],
                        revolveang[i], &dax, &day);
            dragpoint(k, dax, day);
        }
    }
    ...
    for (i = 0; i < subwaytrackcnt; i++)
    {
        ...
        if ((subwayvel[i] <= 2) && (subwayvel[i] >= -2) && (klabs(k) < 2048))
        {
            ...
            if ((subwaypausetime[i] == 720) ||
                ((subwaypausetime[i] >= 120) && (subwaypausetime[i] - TICSPERFRAME < 120)))
...
void statuslistcode(void)
{
    ...
    for (i = headspritestat[1]; i >= 0; i = nexti)
    {
        ...
        for (p = connecthead; p >= 0; p = connectpoint2[p])
        {
            ...
            if (dist < mindist)
                mindist = dist, target = p;
        }

        // brown monster decides to shoot bullet
        if ((k & 63) == 23)
        {
            if (cansee(sprite[i].x, sprite[i].y, sprite[i].z - (tilesizy[sprite[i].picnum] << 7), sprite[i].sectnum,
                       posx[target], posy[target], posz[target], cursectnum[target]) == 0)
            {
                if ((k & 0xf00) == 0xb00)
            ...
            else
            {
                ...
                clipmove(&ox, &oy, &posz[target], &cursectnum[target], xvect << 14, yvect << 14, 128L, 4 << 8, 4 << 8,
                         CLIPMASK0);
                ox -= sprite[i].x;
                oy -= sprite[i].y;
                ...
                spawnsprite(j, sprite[i].x, sprite[i].y, sprite[i].z, 128, 0, 0, 16, sprite[i].xrepeat,
                            sprite[i].yrepeat, 0, 0, BULLET, daang, dax, day, daz, i, sprite[i].sectnum, 6, 0, 0, 0);
        ...
        movestat = movesprite((short)i, (int)sintable[(sprite[i].ang + 512) & 2047] * doubvel,
                              (int)sintable[sprite[i].ang] * doubvel, 0L, 4L << 8, 4L << 8, CLIPMASK0);
        ...
        if ((movestat != 0) || ((k & 63) == 1))
        {
            ...
            if ((sprite[i].extra - sprite[i].ang) & 1024)
            {
                ...
                if (!((sprite[i].extra - sprite[i].ang) & 1024))
                    sprite[i].ang = (sprite[i].extra & 2047);
            }
            else
            {
                sprite[i].ang = ((sprite[i].ang + 32) & 2047);
    ...
    for (i = headspritestat[10]; i >= 0; i = nexti) // EVILAL list
    {
        ...
        if ((sprite[i].lotag & 32) && (k < 48)) // Al decides to reproduce
        {
            ...
            if ((sprite[i].lotag & 64) && (k < 2)) // Give him a chance to reproduce without seeing you
                l = 1;
            else if (cansee(sprite[i].x, sprite[i].y, sprite[i].z - (tilesizy[sprite[i].picnum] << 7),
                            sprite[i].sectnum, posx[target], posy[target], posz[target], cursectnum[target]) == 1)
                l = 1;
            if (l != 0)
            {
                spawnsprite(j, sprite[i].x, sprite[i].y, sprite[i].z, sprite[i].cstat, sprite[i].shade, sprite[i].pal,
                            sprite[i].clipdist, 38, 38, sprite[i].xoffset, sprite[i].yoffset, sprite[i].picnum,
                            krand() & 2047, 0, 0, 0, i, sprite[i].sectnum, 10, sprite[i].lotag, sprite[i].hitag,
                            sprite[i].extra);
                ...
                case 6:
                case 7:
                    sprite[i].lotag ^= (1 << (krand() & 7));
                    break;
                case 8:
                    sprite[i].lotag = (krand() & 255);
                    break;
                }
            }
        }
        ...
        {
            if (cansee(sprite[i].x, sprite[i].y, sprite[i].z - (tilesizy[sprite[i].picnum] << 7), sprite[i].sectnum,
                       posx[target], posy[target], posz[target], cursectnum[target]) == 1)
            {
                wsayfollow("zipguns.wav", 5144L + (krand() & 127) - 64, 256L, &sprite[i].x, &sprite[i].y, 1);

                spawnsprite(j, sprite[i].x, sprite[i].y, sector[sprite[i].sectnum].floorz - (24 << 8), 0, 0, 0, 16, 32,
                            32, 0, 0, BULLET,
                            (getangle(posx[target] - sprite[j].x, posy[target] - sprite[j].y) + (krand() & 15) - 8) &
                                2047,
                            sintable[(sprite[j].ang + 512) & 2047] >> 6, sintable[sprite[j].ang & 2047] >> 6,
                            ((posz[target] + (8 << 8) - sprite[j].z) << 8) /
                                (ksqrt((posx[target] - sprite[j].x) * (posx[target] - sprite[j].x) +
                                       (posy[target] - sprite[j].y) * (posy[target] - sprite[j].y)) +
                                 1),
        ...
        {
            if (((k & 124) >= 120) &&
                (cansee(sprite[i].x, sprite[i].y, sprite[i].z - (tilesizy[sprite[i].picnum] << 7), sprite[i].sectnum,
                        posx[target], posy[target], posz[target], cursectnum[target]) == 1))
    ...
    for (i = headspritestat[6]; i >= 0; i = nexti)
    {
        ...
        if ((nummoves - i) & statrate[6])
            continue;

        // If the sprite is a bullet then...
        if ((sprite[i].picnum == BULLET) || (sprite[i].picnum == GRABBER) || (sprite[i].picnum == MISSILE) ||
            (sprite[i].picnum == BOMB))
        {
            ...
            if (sprite[i].picnum == GRABBER)
            { // Andy did this (& Ken) !Homing!
                ...
                for (j = connecthead; j >= 0; j = connectpoint2[j]) // Players
                    if (j != (sprite[i].owner & (MAXSPRITES - 1)))
                        if (cansee(sprite[i].x, sprite[i].y, sprite[i].z, sprite[i].sectnum, posx[j], posy[j], posz[j],
                                   cursectnum[j]))
                        {
                            k = ksqrt(sqr(posx[j] - sprite[i].x) + sqr(posy[j] - sprite[i].y) +
                                      (sqr(posz[j] - sprite[i].z) >> 8));
                            if (k < l)
                            {
                                l = k;
                ...
                { // Active monsters
                    ...
                    if (cansee(sprite[i].x, sprite[i].y, sprite[i].z, sprite[i].sectnum, sprite[j].x, sprite[j].y,
                               sprite[j].z, sprite[j].sectnum))
                    {
                        k = ksqrt(sqr(sprite[j].x - sprite[i].x) + sqr(sprite[j].y - sprite[i].y) +
                                  (sqr(sprite[j].z - sprite[i].z) >> 8));
                        if (k < l)
                        {
                            l = k;
                ...
                for (j = headspritestat[2]; j >= 0; j = nextj)
                { // Inactive monsters
                    nextj = nextspritestat[j];
                    if (cansee(sprite[i].x, sprite[i].y, sprite[i].z, sprite[i].sectnum, sprite[j].x, sprite[j].y,
                               sprite[j].z, sprite[j].sectnum))
                ...
                if (l != 0x7fffffff)
                {
                    ...
                    l = ksqrt((sprite[i].xvel * sprite[i].xvel) + (sprite[i].yvel * sprite[i].yvel) +
                              ((sprite[i].zvel * sprite[i].zvel) >> 8));
            ...
            if ((hitobject & 0xc000) == 16384) // Bullet hit a ceiling/floor
            {
                ...
                if (sprite[i].z < ((k + l) >> 1))
                    k = sector[hitobject & (MAXSECTORS - 1)].ceilingheinum;
                else
                    k = sector[hitobject & (MAXSECTORS - 1)].floorheinum;
            ...
            else if ((hitobject & 0xc000) == 32768) // Bullet hit a wall
            {
                if (wall[hitobject & 4095].lotag == 8)
                {
                    ...
                    if ((sprite[i].picnum != BOMB) || (dax * dax + day * day >= 512))
                    {
                        ...
                        oy -= sprite[i].yvel;
                        dist = ((ox * ox + oy * oy) >> 8);
                        wsayfollow("bouncy.wav", 4096L + (krand() & 127) - 64, min(dist, 256), &sprite[i].x,
                                   &sprite[i].y, 1);
                        hitobject = 0;
                        sprite[i].owner = -1; // Bullet turns evil!
                    }
            ...
            else if ((hitobject & 0xc000) == 49152) // Bullet hit a sprite
            {
                if (sprite[hitobject & 4095].picnum == BOUNCYMAT)
                {
                    ...
                    sprite[i].owner = -1; // Bullet turns evil!
                    wsayfollow("bouncy.wav", 4096L + (krand() & 127) - 64, min(dist, 256), &sprite[i].x, &sprite[i].y,
                               1);
                    hitobject = 0;
                }
            }
            ...
            if (hitobject != 0)
            {
                if ((sprite[i].picnum == MISSILE) || (sprite[i].picnum == BOMB))
                {
                    if ((hitobject & 0xc000) == 49152)
                        ...
                        {
                            wsayfollow("niceshot.wav", 3840L + (krand() & 127) - 64, 256L, &sprite[i].x, &sprite[i].y,
                                       0);
                            deletesprite((short)i);
                            goto bulletisdeletedskip;
                        }
                ...
                else if ((hitobject & 0xc000) == 32768) // Bullet hit a wall
                {
                    if (wall[hitobject & 4095].picnum == KENPICTURE)
                    {
                        ...
                        wsayfollow("hello.wav", 4096L + (krand() & 127) - 64, 256L, &sprite[i].x, &sprite[i].y,
                                   0); // Ken says, "Hello... how are you today!"
                    }
                    else
                        wsayfollow("bullseye.wav", 4096L + (krand() & 127) - 64, 256L, &sprite[i].x, &sprite[i].y, 0);
                ...
                {
                    if ((sprite[hitobject & 4095].lotag == 5) && (sprite[i].picnum == GRABBER))
                    { // Basketball hoop (Andy's addition)
                        ...
                        case 15:
                        case 16:
                            sprite[i].picnum = DIAMONDS;
                            break;
                        case 17:
                        case 18:
                        case 19:
                            sprite[i].picnum = COINSTACK;
                            break;
                        case 20:
                        case 21:
                        case 22:
                        case 23:
                            sprite[i].picnum = GIFTBOX;
                            break;
                        case 24:
                        case 25:
                            sprite[i].picnum = GRABCANNON;
                            break;
                        case 26:
                        case 27:
                            sprite[i].picnum = LAUNCHER;
                            break;
                        case 28:
                        case 29:
                        case 30:
                            sprite[i].picnum = CANNON;
                            break;
                        case 31:
                            sprite[i].picnum = AIRPLANE;
                            break;
                        default:
                            deletesprite((short)i);
                            goto bulletisdeletedskip;
                    ...
                    if (sprite[hitobject & 4095].picnum == PLAYER)
                        for (j = connecthead; j >= 0; j = connectpoint2[j])
                            if (sprite[i].owner != j + 4096)
                                ...
                                {
                                    wsayfollow("ouch.wav", 4096L + (krand() & 127) - 64, 256L, &sprite[i].x,
                                               &sprite[i].y, 0);
                                    if (sprite[i].picnum == GRABBER)
                                    { // Andy did this
                    ...
                    if (sprite[i].owner != j)
                    {
                        ...
                        case BROWNMONSTER:
                            ...
                            if (sprite[j].lotag > 0)
                            {
                                if (sprite[j].lotag <= 25)
                                    sprite[j].cstat |= 2;
                                wsayfollow("hurt.wav", 4096L + (krand() & 511) - 256, 256L, &sprite[i].x, &sprite[i].y,
                                           1);
                            }
                            else
                            {
                                wsayfollow("mondie.wav", 4096L + (krand() & 127) - 64, 256L, &sprite[i].x, &sprite[i].y,
                                           0);
                                sprite[j].z += ((tilesizy[sprite[j].picnum] * sprite[j].yrepeat) << 1);
                                sprite[j].picnum = GIFTBOX;
                                sprite[j].cstat &= ~0x83; // Should not clip, foot-z

                                spawnsprite(k, sprite[j].x, sprite[j].y, sprite[j].z, 0, -4, 0, 32, 64, 64, 0, 0,
                                            EXPLOSION, sprite[j].ang, 0, 0, 0, j, sprite[j].sectnum, 5, 31, 0, 0);
                        ...
                        case AL:
                            ...
                            goto bulletisdeletedskip;
                        default:
                            wsayfollow("bullseye.wav", 4096L + (krand() & 127) - 64, 256L, &sprite[i].x, &sprite[i].y,
                                       0);
                            deletesprite((short)i);
                            goto bulletisdeletedskip;
                        }
    ...
    for (i = headspritestat[2]; i >= 0; i = nexti)
    {
        ...
        for (p = connecthead; p >= 0; p = connectpoint2[p])
            if (sintable[(sprite[i].ang + 512) & 2047] * (posx[p] - sprite[i].x) +
                    sintable[sprite[i].ang & 2047] * (posy[p] - sprite[i].y) >=
                0)
                if (cansee(sprite[i].x, sprite[i].y, sprite[i].z - (tilesizy[sprite[i].picnum] << 7), sprite[i].sectnum,
                           posx[p], posy[p], posz[p], cursectnum[p]) == 1)
                {
...
void activatehitag(short dahitag)
{
    ...
    for (i = 0; i < numsectors; i++)
        if (sector[i].hitag == dahitag)
            operatesector(i);

    for (i = headspritestat[0]; i >= 0; i = nexti)
    {
        nexti = nextspritestat[i];
        if (sprite[i].hitag == dahitag)
            operatesprite(i);
    }
}
...
void bombexplode(int i)
{
    ...
    spawnsprite(j, sprite[i].x, sprite[i].y, sprite[i].z, 0, -4, 0, 32, 64, 64, 0, 0, EXPLOSION, sprite[i].ang, 0, 0, 0,
                sprite[i].owner, sprite[i].sectnum, 5, 31, 0, 0);
    // 31=Time left for explosion to stay

    for (k = 0; k < 12; k++)
    {
        spawnsprite(j, sprite[i].x, sprite[i].y, sprite[i].z + (8 << 8), 2, -4, 0, 32, 24, 24, 0, 0, EXPLOSION,
                    sprite[i].ang, (krand() >> 7) - 256, (krand() >> 7) - 256, (krand() >> 2) - 8192, sprite[i].owner,
                    sprite[i].sectnum, 7, 96, 0, 0);
    ...
    for (j = connecthead; j >= 0; j = connectpoint2[j])
    {
        ...
        if (dist < 4194304)
            if (cansee(sprite[i].x, sprite[i].y, sprite[i].z - (tilesizy[sprite[i].picnum] << 7), sprite[i].sectnum,
                       posx[j], posy[j], posz[j], cursectnum[j]) == 1)
            {
                k = ((32768 / ((dist >> 16) + 4)) >> 5);
                if (j == myconnectindex)
                {
    ...
    for (k = 1; k <= 2; k++) // Check for hurting monsters
    {
        for (j = headspritestat[k]; j >= 0; j = nextj)
        {
            ...
            if (dist >= 4194304)
                continue;
            if (cansee(sprite[i].x, sprite[i].y, sprite[i].z - (tilesizy[sprite[i].picnum] << 7), sprite[i].sectnum,
                       sprite[j].x, sprite[j].y, sprite[j].z - (tilesizy[sprite[j].picnum] << 7),
                       sprite[j].sectnum) == 0)
                continue;
    ...
    for (j = headspritestat[10]; j >= 0; j = nextj) // Check for EVILAL's
    {
        ...
        if (dist >= 4194304)
            continue;
        if (cansee(sprite[i].x, sprite[i].y, sprite[i].z - (tilesizy[sprite[i].picnum] << 7), sprite[i].sectnum,
                   sprite[j].x, sprite[j].y, sprite[j].z - (tilesizy[sprite[j].picnum] << 7), sprite[j].sectnum) == 0)
            continue;
...
void processinput(short snum)
{
    ...
    getzrange(posx[snum], posy[snum], posz[snum], cursectnum[snum], &globhiz, &globhihit, &globloz, &globlohit, 128L,
              CLIPMASK0);
    sprite[playersprite[snum]].cstat |= 1;
    ...
    if (health[snum] < 0)
    {
        ...
        if (health[snum] <= -160)
        {
            ...
            {
                spawnsprite(j, sprite[i].x, sprite[i].y, sprite[i].z + (8 << 8), 2, -4, 0, 32, 24, 24, 0, 0, EXPLOSION,
                            sprite[i].ang, (krand() & 511) - 256, (krand() & 511) - 256, (krand() & 16384) - 8192,
    ...
    if (sector[cursectnum[snum]].lotag == 4) // slime sector
        if ((globlohit & 0xc000) != 49152)   // You're not on a sprite
        {
            ...
            {
                clipmove(&posx[snum], &posy[snum], &posz[snum], &cursectnum[snum], -(TICSPERFRAME << 14),
                         -(TICSPERFRAME << 14), 128L, 4 << 8, 4 << 8, CLIPMASK0);
    ...
    posz[snum] += hvel[snum];
    if (posz[snum] > globloz - (4 << 8))
        posz[snum] = globloz - (4 << 8), hvel[snum] = 0;
    if (posz[snum] < globhiz + (4 << 8))
        posz[snum] = globhiz + (4 << 8), hvel[snum] = 0;

    if (dimensionmode[snum] != 3)
    {
        if (((ssync[snum].bits & 32) > 0) && (zoom[snum] > 48))
            zoom[snum] -= (zoom[snum] >> 4);
        if (((ssync[snum].bits & 16) > 0) && (zoom[snum] < 4096))
            zoom[snum] += (zoom[snum] >> 4);
    }
    ...
    if ((ssync[snum].bits & 1024) > 0) // Space bar
    {
        ...
        neartag(posx[snum], posy[snum], posz[snum], cursectnum[snum], ang[snum], &neartagsector, &neartagwall,
                &neartagsprite, &neartaghitdist, 1024L, 3);
        if (neartagsector == -1)
        {
            i = cursectnum[snum];
        ...
        if ((oflags[snum] & 1024) == 0)
        {
            ...
            if (neartagwall >= 0)
                if (wall[neartagwall].lotag == 2) // Switch
                {
                    ...
                    if (j == SWITCH2ON)
                        wall[neartagwall].overpicnum = SWITCH2OFF;
                    if (j == SWITCH2OFF)
                        wall[neartagwall].overpicnum = SWITCH2ON;
                    if (j == SWITCH3ON)
                        wall[neartagwall].overpicnum = SWITCH3OFF;
                    if (j == SWITCH3OFF)
                        wall[neartagwall].overpicnum = SWITCH3ON;
            ...
            if (neartagsprite >= 0)
            {
                ...
                if (sprite[neartagsprite].lotag == 4)
                {
                    ...
                    if (j == SWITCH2ON)
                        sprite[neartagsprite].picnum = SWITCH2OFF;
                    if (j == SWITCH2OFF)
                        sprite[neartagsprite].picnum = SWITCH2ON;
                    if (j == SWITCH3ON)
                        sprite[neartagsprite].picnum = SWITCH3OFF;
                    if (j == SWITCH3OFF)
                        sprite[neartagsprite].picnum = SWITCH3ON;
...
void drawscreen(short snum, int dasmoothratio)
{
    ...
    if (dimensionmode[myconnectindex] == 3)
    {
        ...
        if (apply)
        {
            ...
            if (screensize <= xdim)
            {
                rotatesprite((xdim - 320) << 15, (ydim - 32) << 16, 65536L, 0, STATUSBAR, 0, 0, 8 + 16 + 64 + 128, 0L,
                             0L, xdim - 1L, ydim - 1L);
                i = ((xdim - 320) >> 1);
                while (i >= 8)
                    i -= 8, rotatesprite(i << 16, (ydim - 32) << 16, 65536L, 0, STATUSBARFILL8, 0, 0, 8 + 16 + 64 + 128,
                                         0L, 0L, xdim - 1L, ydim - 1L);
                if (i >= 4)
                    i -= 4, rotatesprite(i << 16, (ydim - 32) << 16, 65536L, 0, STATUSBARFILL4, 0, 0, 8 + 16 + 64 + 128,
                                         0L, 0L, xdim - 1L, ydim - 1L);
                i = ((xdim - 320) >> 1) + 320;
                while (i <= xdim - 8)
                    rotatesprite(i << 16, (ydim - 32) << 16, 65536L, 0, STATUSBARFILL8, 0, 0, 8 + 16 + 64 + 128, 0L, 0L,
                                 xdim - 1L, ydim - 1L),
                        i += 8;
                if (i <= xdim - 4)
                    rotatesprite(i << 16, (ydim - 32) << 16, 65536L, 0, STATUSBARFILL4, 0, 0, 8 + 16 + 64 + 128, 0L, 0L,
                                 xdim - 1L, ydim - 1L),
                        i += 4;
            ...
            for (j = (numplayers >> 2) + 1; j > 0; j--)
            {
                ...
                for (i = connecthead; i >= 0; i = connectpoint2[i])
                    if (frame2draw[i] == 0)
                    {
                        ...
                        if ((posx[i] != oposx[i]) || (posy[i] != oposy[i]) || (posz[i] != oposz[i]) ||
                            (ang[i] != oang[i]) || (horiz[i] != ohoriz[i]))
                            dist += dist;
            ...
            for (i = connecthead, j = 0; i >= 0; i = connectpoint2[i], j++)
                if (frame2draw[i] != 0)
                {
                    if (numplayers <= 4)
                    {
                        ...
                        {
                        case 0:
                            setview(0, 0, (xdim >> 1) - 1, (ydim >> 1) - 1);
                            break;
                        case 1:
                            setview((xdim >> 1), 0, xdim - 1, (ydim >> 1) - 1);
                            break;
                        case 2:
                            setview(0, (ydim >> 1), (xdim >> 1) - 1, ydim - 1);
                            break;
                        case 3:
                            setview((xdim >> 1), (ydim >> 1), xdim - 1, ydim - 1);
                            break;
                        }
                    }
                    else
                    {
                        ...
                        {
                        case 0:
                            setview(0, 0, (xdim >> 2) - 1, (ydim >> 2) - 1);
                            break;
                        case 1:
                            setview(xdim >> 2, 0, (xdim >> 1) - 1, (ydim >> 2) - 1);
                            break;
                        case 2:
                            setview(xdim >> 1, 0, xdim - (xdim >> 2) - 1, (ydim >> 2) - 1);
                            break;
                        case 3:
                            setview(xdim - (xdim >> 2), 0, xdim - 1, (ydim >> 2) - 1);
                            break;
                        case 4:
                            setview(0, ydim >> 2, (xdim >> 2) - 1, (ydim >> 1) - 1);
                            break;
                        case 5:
                            setview(xdim >> 2, ydim >> 2, (xdim >> 1) - 1, (ydim >> 1) - 1);
                            break;
                        case 6:
                            setview(xdim >> 1, ydim >> 2, xdim - (xdim >> 2) - 1, (ydim >> 1) - 1);
                            break;
                        case 7:
                            setview(xdim - (xdim >> 2), ydim >> 2, xdim - 1, (ydim >> 1) - 1);
                            break;
                        case 8:
                            setview(0, ydim >> 1, (xdim >> 2) - 1, ydim - (ydim >> 2) - 1);
                            break;
                        case 9:
                            setview(xdim >> 2, ydim >> 1, (xdim >> 1) - 1, ydim - (ydim >> 2) - 1);
                            break;
                        case 10:
                            setview(xdim >> 1, ydim >> 1, xdim - (xdim >> 2) - 1, ydim - (ydim >> 2) - 1);
                            break;
                        case 11:
                            setview(xdim - (xdim >> 2), ydim >> 1, xdim - 1, ydim - (ydim >> 2) - 1);
                            break;
                        case 12:
                            setview(0, ydim - (ydim >> 2), (xdim >> 2) - 1, ydim - 1);
                            break;
                        case 13:
                            setview(xdim >> 2, ydim - (ydim >> 2), (xdim >> 1) - 1, ydim - 1);
                            break;
                        case 14:
                            setview(xdim >> 1, ydim - (ydim >> 2), xdim - (xdim >> 2) - 1, ydim - 1);
                            break;
                        case 15:
                            setview(xdim - (xdim >> 2), ydim - (ydim >> 2), xdim - 1, ydim - 1);
                            break;
                        }
                    }
                    ...
                    if ((numgrabbers[i] > 0) || (nummissiles[i] > 0) || (numbombs[i] > 0))
                        rotatesprite(160 << 16, 184L << 16, 65536, 0, GUNONBOTTOM, sector[cursectnum[i]].floorshade, 0,
                                     2, windowx1, windowy1, windowx2, windowy2);

                    if (lockclock < 384)
                    {
                        if (lockclock < 128)
                            rotatesprite(320 << 15, 200 << 15, lockclock << 9, lockclock << 4, DEMOSIGN,
                                         (128 - lockclock) >> 2, 0, 1 + 2, windowx1, windowy1, windowx2, windowy2);
                        else if (lockclock < 256)
                            rotatesprite(320 << 15, 200 << 15, 65536, 0, DEMOSIGN, 0, 0, 2, windowx1, windowy1,
                                         windowx2, windowy2);
                        else
                            rotatesprite(320 << 15, 200 << 15, (384 - lockclock) << 9, lockclock << 4, DEMOSIGN,
                                         (lockclock - 256) >> 2, 0, 1 + 2, windowx1, windowy1, windowx2, windowy2);
                    }

                    if (health[i] <= 0)
                        rotatesprite(320 << 15, 200 << 15, (-health[i]) << 11, (-health[i]) << 5, NO, 0, 0, 2, windowx1,
        ...
        else
        {
            ...
            if (intptr[0] | intptr[1])
                for (i = MAXMIRRORS - 1; i >= 0; i--)
                    if (gotpic[(i + MIRRORLABEL) >> 3] & (1 << (i & 7)))
                    {
                        ...
                        preparemirror(cposx, cposy, cposz, cang, choriz, mirrorwall[i], mirrorsector[i], &tposx, &tposy,
                                      &tang);
            ...
            if (POLYMOST_RENDERMODE_CLASSIC())
            {
                if ((tiltlock) || (detailmode))
                {
                    ...
                    if (detailmode == 0)
                        i >>= 1;
                    rotatesprite(320 << 15, 200 << 15, i, tiltlock + 512, MAXTILES - 2, 0, 0, 2 + 4 + 64, windowx1,
                                 windowy1, windowx2, windowy2);
                    walock[MAXTILES - 2] = 1;
                }
            }

            if (((numgrabbers[screenpeek] > 0) || (nummissiles[screenpeek] > 0) || (numbombs[screenpeek] > 0)) &&
                (cameradist < 0))
            {
                ...
                rotatesprite(160 << 16, 184L << 16, 65536, 0, GUNONBOTTOM, sector[cursectnum[screenpeek]].floorshade, 0,
                             2, windowx1, windowy1, windowx2, windowy2);
            }

            if (cachecount != 0)
            {
                rotatesprite((320 - 16) << 16, 16 << 16, 32768, 0, BUILDDISK, 0, 0, 2 + 64, windowx1, windowy1,
                             windowx2, windowy2);
                cachecount = 0;
            }
            ...
            {
                if (lockclock < 128)
                    rotatesprite(320 << 15, 200 << 15, lockclock << 9, lockclock << 4, DEMOSIGN, (128 - lockclock) >> 2,
                                 0, 1 + 2, windowx1, windowy1, windowx2, windowy2);
                else if (lockclock < 256)
                    rotatesprite(320 << 15, 200 << 15, 65536, 0, DEMOSIGN, 0, 0, 2, windowx1, windowy1, windowx2,
                                 windowy2);
                else
                    rotatesprite(320 << 15, 200 << 15, (384 - lockclock) << 9, lockclock << 4, DEMOSIGN,
                                 (lockclock - 256) >> 2, 0, 1 + 2, windowx1, windowy1, windowx2, windowy2);
    ...
    if (getmessageleng > 0)
    {
        ...
        for (i = 0; i <= getmessageleng; i += charsperline)
        {
            ...
            printext256(0L, ((i / charsperline) << 3) + (ydim - 32 - 8) - (((getmessageleng - 1) / charsperline) << 3),
                        31 /*151*/, -1, (char *)tempbuf, 0);
        }
    ...
    if (keystatus[0x3e]) // F4 - screen re-size
    {
        ...
        else
        {
            ...
            for (i = 0; i < validmodecnt; i++)
                if ((validmode[i].xdim == xdim) && (validmode[i].ydim == ydim) && (validmode[i].fs == fullscreen) &&
                    (validmode[i].bpp == bpp))
                {
                    j = i;
                    break;
...
void fakedomovethings(void)
{
    ...
    if (((syn->bits & 8) > 0) && (myhoriz > 100 - (200 >> 1)))
        myhoriz -= 4; //-
    if (((syn->bits & 4) > 0) && (myhoriz < 100 + (200 >> 1)))
        myhoriz += 4; //+
    ...
    if (sector[mycursectnum].lotag == 4)   // slime sector
        if ((globlohit & 0xc000) != 49152) // You're not on a sprite
        {
            goalz = globloz - (8 << 8);
            if (myz >= goalz - (2 << 8))
                clipmove(&myx, &myy, &myz, &mycursectnum, -(TICSPERFRAME << 14), -(TICSPERFRAME << 14), 128L, 4 << 8,
                         4 << 8, CLIPMASK0);
        }
    if (goalz < globhiz + (16 << 8)) // ceiling&floor too close
        goalz = ((globloz + globhiz) >> 1);
    ...
    myz += myzvel;
    if (myz > globloz - (4 << 8))
        myz = globloz - (4 << 8), myzvel = 0;
    if (myz < globhiz + (4 << 8))
        myz = globhiz + (4 << 8), myzvel = 0;
...
void fakedomovethingscorrect(void)
{
    ...
    if ((posx[myconnectindex] == myxbak[i]) && (posy[myconnectindex] == myybak[i]) &&
        (posz[myconnectindex] == myzbak[i]) && (horiz[myconnectindex] == myhorizbak[i]) &&
...
void getinput(void)
{
    ...
    if ((option[4] == 0) && (numplayers >= 2))
    {
        ...
        if (svel2 > 0)
            svel2 = max(svel2 - 2 * TICSPERFRAME, 0);
        if (fvel2 < 0)
            fvel2 = min(fvel2 + 2 * TICSPERFRAME, 0);
        if (fvel2 > 0)
...
void initplayersprite(short snum)
{
    ...
    spawnsprite(playersprite[snum], posx[snum], posy[snum], posz[snum] + EYEHEIGHT, 1 + 256, 0, snum, 32, 64, 64, 0, 0,
                PLAYER, ang[snum], 0, 0, 0, snum + 4096, cursectnum[snum], 8, 0, 0, 0);
...
void setup3dscreen(void)
{
    ...
    if (screensize <= xdim)
    {
        rotatesprite((xdim - 320) << 15, (ydim - 32) << 16, 65536L, 0, STATUSBAR, 0, 0, 8 + 16 + 64 + 128, 0L, 0L,
                     xdim - 1L, ydim - 1L);
        i = ((xdim - 320) >> 1);
        while (i >= 8)
            i -= 8, rotatesprite(i << 16, (ydim - 32) << 16, 65536L, 0, STATUSBARFILL8, 0, 0, 8 + 16 + 64 + 128, 0L, 0L,
                                 xdim - 1L, ydim - 1L);
        if (i >= 4)
            i -= 4, rotatesprite(i << 16, (ydim - 32) << 16, 65536L, 0, STATUSBARFILL4, 0, 0, 8 + 16 + 64 + 128, 0L, 0L,
                                 xdim - 1L, ydim - 1L);
        i = ((xdim - 320) >> 1) + 320;
        while (i <= xdim - 8)
            rotatesprite(i << 16, (ydim - 32) << 16, 65536L, 0, STATUSBARFILL8, 0, 0, 8 + 16 + 64 + 128, 0L, 0L,
...
void findrandomspot(int *x, int *y, short *sectnum)
{
    ...
    for (cnt = 256; cnt >= 0; cnt--)
    {
        ...
        } while ((sector[dasector].ceilingz + (8 << 8) >= sector[dasector].floorz) ||
                 ((sector[dasector].lotag | sector[dasector].hitag) != 0) || ((sector[dasector].floorstat & 1) != 0));
        ...
        if (inside(dax, day, dasector) == 0)
            continue;

        daz = sector[dasector].floorz - (32 << 8);
        if (pushmove(&dax, &day, &daz, &dasector, 128L, 4 << 8, 4 << 8, CLIPMASK0) < 0)
            continue;
...
#pragma aux addlava = "mov al, byte ptr [ebx-133]", "mov dl, byte ptr [ebx-1]", "add al, byte ptr [ebx-132]",          \
            "add dl, byte ptr [ebx+131]", "add al, byte ptr [ebx-131]", "add dl, byte ptr [ebx+132]",                  \
            "add al, byte ptr [ebx+1]", "add al, dl", parm[ebx] modify exact[eax edx]
int addlava(int);
...
{
    int r;
    __asm__ __volatile__("movb -133(%%ebx), %%al\n\t"
                         "movb -1(%%ebx), %%dl\n\t"
                         "addb -132(%%ebx), %%al\n\t"
                         "addb 131(%%ebx), %%dl\n\t"
                         "addb -131(%%ebx), %%al\n\t"
...
void movelava(unsigned char *dapic)
{
    ...
    for (x = 0; x < LAVASIZ; x++)
    {
        ...
        {
            *ptr++ = ((addlava(&py[0]) & 0xf8) >> 3) + ((addlava(&py[1]) & 0xf8) << 5) +
                     ((addlava(&py[2]) & 0xf8) << 13) + ((addlava(&py[3]) & 0xf8) << 21) + 0xc2c2c2c2;
        }
    }
...
int loadgame(void)
{
    ...
    if ((fil = kopen4load("save0000.gam", 0)) == -1)
        return (-1);
...
int savegame(void)
{
    ...
    if ((fil = Bfopen("save0000.gam", "wb")) == 0)
        return (-1);
...
void faketimerhandler(void)
{
    ...
    sampletimer();
    if ((totalclock < ototalclock + (TIMERINTSPERSECOND / MOVESPERSECOND)) || (ready2send == 0))
        return;
    ...
    */
    ...
    if (networkmode == 1)
    {
        packbuf[2] = 0;
        j = 3;
        if (loc.fvel != oloc.fvel)
            packbuf[j++] = loc.fvel, packbuf[2] |= 1;
        if (loc.svel != oloc.svel)
            packbuf[j++] = loc.svel, packbuf[2] |= 2;
        if (loc.avel != oloc.avel)
            packbuf[j++] = loc.avel, packbuf[2] |= 4;
        if ((loc.bits ^ oloc.bits) & 0x00ff)
            packbuf[j++] = (loc.bits & 255), packbuf[2] |= 8;
        if ((loc.bits ^ oloc.bits) & 0xff00)
            packbuf[j++] = ((loc.bits >> 8) & 255), packbuf[2] |= 16;
        ...
        for (i = connecthead; i >= 0; i = connectpoint2[i])
            if (i != myconnectindex)
            {
                ...
                k = j;
                if ((myconnectindex == connecthead) ||
                    ((i == connecthead) && (myconnectindex == connectpoint2[connecthead])))
                {
    ...
    if ((myconnectindex == connecthead) || (option[4] == 0))
    {
        ...
        if (option[4] != 0)
        {
            ...
            {
                l = 0;
                if (ffsync[i].fvel != osync[i].fvel)
                    packbuf[j++] = ffsync[i].fvel, l |= 1;
                if (ffsync[i].svel != osync[i].svel)
                    packbuf[j++] = ffsync[i].svel, l |= 2;
                if (ffsync[i].avel != osync[i].avel)
                    packbuf[j++] = ffsync[i].avel, l |= 4;
    ...
    else // I am a SLAVE
    {
        ...
        j = 2;
        if (loc.fvel != oloc.fvel)
            packbuf[j++] = loc.fvel, packbuf[1] |= 1;
        if (loc.svel != oloc.svel)
            packbuf[j++] = loc.svel, packbuf[1] |= 2;
        if (loc.avel != oloc.avel)
            packbuf[j++] = loc.avel, packbuf[1] |= 4;
        if ((loc.bits ^ oloc.bits) & 0x00ff)
            packbuf[j++] = (loc.bits & 255), packbuf[1] |= 8;
        if ((loc.bits ^ oloc.bits) & 0xff00)
            packbuf[j++] = ((loc.bits >> 8) & 255), packbuf[1] |= 16;
        copybufbyte(&loc, &oloc, sizeof(input));
    ...
    while ((packbufleng = getpacket(&other, packbuf)) > 0)
    {
        ...
        case 0: //[0] (receive master sync buffer)
            ...
            for (i = connecthead; i >= 0; i = connectpoint2[i])
            {
                l = (packbuf[k >> 3] >> (k & 7));
                if (l & 1)
                    ffsync[i].fvel = packbuf[j++];
                if (l & 2)
                    ffsync[i].svel = packbuf[j++];
                if (l & 4)
                    ffsync[i].avel = packbuf[j++];
                if (l & 8)
                {
        ...
        case 1: //[1] (receive slave sync buffer)
            ...
            k = packbuf[1];
            if (k & 1)
                ffsync[other].fvel = packbuf[j++];
            if (k & 2)
                ffsync[other].svel = packbuf[j++];
            if (k & 4)
                ffsync[other].avel = packbuf[j++];
            if (k & 8)
                ffsync[other].bits = ((ffsync[other].bits & 0xff00) | ((short)packbuf[j++]));
            if (k & 16)
                ffsync[other].bits = ((ffsync[other].bits & 0x00ff) | (((short)packbuf[j++]) << 8));
            break;
        case 2:
            getmessageleng = packbufleng - 1;
            for (j = getmessageleng - 1; j >= 0; j--)
                getmessage[j] = packbuf[j + 1];
            getmessagetimeoff = totalclock + 360 + (getmessageleng << 4);
            ...
            if ((show2dsprite[j >> 3] & (1 << (j & 7))) > 0)
            {
                ...
                if (k >= 0)
                {
                    ...
                    case 0:
                        ...
                        break;
                    case 1:
                        l = (smoothratio >> 1) + (((nummoves - j) & 1) << 15);
                        break;
                    case 3:
                        l = (smoothratio >> 2) + (((nummoves - j) & 3) << 14);
                        break;
                    case 7:
                        l = (smoothratio >> 3) + (((nummoves - j) & 7) << 13);
                        break;
                    case 15:
                        l = (smoothratio >> 4) + (((nummoves - j) & 15) << 12);
                        break;
                    }
                ...
                case 0:
                    ...
                    if (dimensionmode[screenpeek] == 1)
                    {
                        ...
                        drawline256(x1 - x2 + (xdim << 11), y1 - y3 + (ydim << 11), x1 + x2 + (xdim << 11),
                                    y1 + y3 + (ydim << 11), col);
                        drawline256(x1 - y2 + (xdim << 11), y1 + x3 + (ydim << 11), x1 + x2 + (xdim << 11),
                                    y1 + y3 + (ydim << 11), col);
                        drawline256(x1 + y2 + (xdim << 11), y1 - x3 + (ydim << 11), x1 + x2 + (xdim << 11),
                                    y1 + y3 + (ydim << 11), col);
                    }
                    else
                    {
                        if (((gotsector[i >> 3] & (1 << (i & 7))) > 0) && (czoom > 96))
                        {
                            ...
                            rotatesprite((x1 << 4) + (xdim << 15), (y1 << 4) + (ydim << 15),
                                         mulscale16(czoom * spr->yrepeat, yxaspect), daang, spr->picnum, spr->shade,
                                         spr->pal, (spr->cstat & 2) >> 1, windowx1, windowy1, windowx2, windowy2);
                        }
                    }
                    break;
                case 16:
...
int movesprite(short spritenum, int dx, int dy, int dz, int ceildist, int flordist, int clipmask)
{
    ...
    retval =
        clipmove(&spr->x, &spr->y, &daz, &dasectnum, dx, dy, ((int)spr->clipdist) << 2, ceildist, flordist, clipmask);
    ...
    spr->cstat &= ~1;
    getzrange(spr->x, spr->y, spr->z - 1, spr->sectnum, &globhiz, &globhihit, &globloz, &globlohit,
...
void waitforeverybody()
{
    ...
    while (1)
    {
        ...
        drawrooms(posx[myconnectindex], posy[myconnectindex], posz[myconnectindex], ang[myconnectindex],
                  horiz[myconnectindex], cursectnum[myconnectindex]);
        if (!networkmode)
            Bsprintf((char *)tempbuf, "Master/slave mode");
        else
            Bsprintf((char *)tempbuf, "Peer-peer mode");
...
void printext(int x, int y, char *buffer, short tilenum, unsigned char invisiblecol)
{
    ...
    for (i = 0; buffer[i] != 0; i++)
    {
        ...
        rotatesprite((x - ((8 & 15) << 3)) << 16, (y - ((8 >> 4) << 3)) << 16, 65536L, 0, tilenum, 0, 0,
                     8 + 16 + 64 + 128, x, y, x + 7, y + 7);
        rotatesprite((x - ((ch & 15) << 3)) << 16, (y - ((ch >> 4) << 3)) << 16, 65536L, 0, tilenum, 0, 0, 8 + 16 + 128,
                     x, y, x + 7, y + 7);
        x += 8;
    }
}

void drawtilebackground(int thex, int they, short tilenum, signed char shade, int cx1, int cy1, int cx2, int cy2,
                        unsigned char dapalnum)
{
    int x, y, xsiz, ysiz, tx1, ty1, tx2, ty2;

    xsiz = tilesizx[tilenum];
    tx1 = cx1 / xsiz;
    tx2 = cx2 / xsiz;
    ysiz = tilesizy[tilenum];
    ty1 = cy1 / ysiz;
    ty2 = cy2 / ysiz;

    for (x = tx1; x <= tx2; x++)
        for (y = ty1; y <= ty2; y++)
            rotatesprite(x * xsiz << 16, y * ysiz << 16, 65536L, 0, tilenum, shade, dapalnum, 8 + 16 + 64 + 128, cx1,
                         cy1, cx2, cy2);
}
