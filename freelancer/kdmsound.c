...
int initkdm(char dadigistat, char damusistat, int dasamplerate, char danumspeakers, char dabytespersample)
{
    ...
    for (i = 0; i < 256; i++)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            eff[5][i] = 65536 * 330 / 262;
            break;
        case 2:
            eff[5][i] = 65536 * 392 / 262;
            break;
        }
...
void preparekdmsndbuf(unsigned char *sndoffsplc, int sndbufsiz)
{
    ...
    for (dacnt = 0; dacnt < sndbufsamples; dacnt += bytespertic)
    {
        if (musicstatus > 0) // Gets here 120 times/second
        {
            while ((notecnt < numnotes) && (timecount >= nttime[notecnt]))
            {
                ...
                if (ntvol1[notecnt] == 0) // Note off
                {
                    ...
                    startwave(j, k, ntvol1[notecnt], ntvol2[notecnt], ntfrqeff[notecnt], ntvoleff[notecnt],
                              ntpaneff[notecnt]);
        ...
        {
            if (bytespersample == 1)
                bound2char(bytespertic >> 1, stemp, (unsigned char *)&sndoffsplc[dacnt]);
            else
                bound2short(bytespertic >> 1, stemp, (short *)&sndoffsplc[dacnt << 1]);
        }
        else
        {
            if (bytespersample == 1)
                bound2char(bytespertic, stemp, (unsigned char *)&sndoffsplc[dacnt << 1]);
            else
                bound2short(bytespertic, stemp, (short *)&sndoffsplc[dacnt << 2]);
        }
    }
}
