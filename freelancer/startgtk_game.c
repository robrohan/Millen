...
static GObject *get_and_connect_signal(GtkBuilder *builder, const char *name, const char *signal_name,
                                       GCallback handler)
{
    GObject *object;
...
static void populate_video_modes(gboolean firsttime)
{
    ...
    for (i = 0; i < validmodecnt; i++)
    {
        ...
        sprintf(modestr, "%d \xc3\x97 %d %d-bpp", validmode[i].xdim, validmode[i].ydim, validmode[i].bpp);
        gtk_list_store_insert_with_values(controls.vmode3dlist, &iter, -1, 0, modestr, 1, i, -1);
        if (i == mode3d)
        {
...
static void setup_config_mode(void)
{
    ...
    gtk_container_foreach(GTK_CONTAINER(controls.configbox), (GtkCallback)gtk_widget_set_sensitive, (gpointer)TRUE);
...
static void setup_messages_mode(gboolean allowcancel)
{
    ...
    gtk_container_foreach(GTK_CONTAINER(controls.configbox), (GtkCallback)gtk_widget_set_sensitive, (gpointer)FALSE);
...
static GtkWindow *create_window(void)
{
    ...
    window =
        GTK_WIDGET(get_and_connect_signal(builder, "startgtk", "delete-event", G_CALLBACK(on_startgtk_delete_event)));
    if (!window)
    {
        goto fail;
    }
    ...
    controls.startbutton =
        GTK_WIDGET(get_and_connect_signal(builder, "startbutton", "clicked", G_CALLBACK(on_startbutton_clicked)));
    controls.cancelbutton =
        GTK_WIDGET(get_and_connect_signal(builder, "cancelbutton", "clicked", G_CALLBACK(on_cancelbutton_clicked)));
    ...
    controls.fullscreencheck = GTK_WIDGET(
        get_and_connect_signal(builder, "fullscreencheck", "toggled", G_CALLBACK(on_fullscreencheck_toggled)));

    controls.singleplayerbutton = GTK_WIDGET(
        get_and_connect_signal(builder, "singleplayerbutton", "toggled", G_CALLBACK(on_multiplayerradio_toggled)));
    controls.joinmultibutton = GTK_WIDGET(
        get_and_connect_signal(builder, "joinmultibutton", "toggled", G_CALLBACK(on_multiplayerradio_toggled)));
    controls.hostmultibutton = GTK_WIDGET(
        get_and_connect_signal(builder, "hostmultibutton", "toggled", G_CALLBACK(on_multiplayerradio_toggled)));
