...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    printf("KEXTRACT [grouped file][@file or filespec...]           by Kenneth "
           "Silverman\n");
    printf("   This program extracts files from a previously grouped group "
           "file.\n");
    printf("   You can extract files using the ? and * wildcards.\n");
    printf("   Ex: kextract stuff.dat tiles000.art nukeland.map palette.dat\n");
    printf("      (stuff.dat is the group file, the rest are the files to "
           "extract)\n");
    return (0);
  }
  ...
  for (i = 0; i < numfiles; i++) {
    if (marked4extraction[i] == 0)
      continue;

    fileleng[i] = fileoffs[i + 1] - fileoffs[i];

    if ((fil2 = Bopen(filelist[i], BO_BINARY | BO_TRUNC | BO_CREAT | BO_WRONLY,
